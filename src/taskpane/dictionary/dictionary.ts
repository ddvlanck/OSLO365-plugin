...
function createSearchResultItemHtml(description: string, referenceUrl: string): string {
  let html = "";
...
function loadDictionary() {
  ...
  for (let i = 0; i < myDictionary.length; i++) {
    const deleteBtn =
      "<img class='dictionary-deletebtn' id='dictionary-deletebtn' style='float:left;' src='assets/deleteBtn.png' width='16px' data-id='" +
      i +
      "'>";

    document.getElementById("ResultBox").innerHTML +=
      "<h3 class='mydict-word'>" + myDictionary[i][0].toLowerCase() + "</h3>";
    ...
    document.getElementById("ResultBox").innerHTML += createSearchResultItemHtml(
      myDictionary[i][1].description,
      myDictionary[i][1].reference
    );
  }
